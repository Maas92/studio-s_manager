...
export const CartItemSchema = z.object({
  ...
  locked: z.boolean().optional(), // Staff assignment locked
});
...
export async function getTodaysAppointments(): Promise<any[]> {
  try {
    const today = new Date().toISOString().split('T')[0];
    const { data } = await api.get(`/appointments?date=${today}&status=confirmed`);
...
// Get available products for retail with stock
export async function getRetailProducts(): Promise<any[]> {
  try {
...
export async function getAvailableTreatments(): Promise<any[]> {
  try {
...
// Get available staff members with schedule
export async function getAvailableStaff(date?: string): Promise<any[]> {
  try {
...
// Create a new client
export async function createClient(input: CreateClientInput): Promise<any> {
  try {
...
export function validateAddToCart(
  ...
  // Only appointments cannot be duplicated (same appointment can't be checked in twice)
  // Treatments CAN be duplicated (e.g., paying for 2 pedicures)
...
export function detectStaffConflicts(
  ...
  cart
    .filter((item) => (item.type === 'treatment' || item.type === 'appointment') && item.staffId)
    .forEach((item) => {
  ...
  staffMap.forEach((items, staffId) => {
    if (items.length > 1) {
      ...
      conflicts.push({
        ...
        overlappingTime: true, // Simplified - in production, check actual times
      });
    }
  });
...
export async function createTransaction(
  ...
  try {
    ...
    const discountAmount = validatedInput.discount.type === 'percentage'
